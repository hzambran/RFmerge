...
Finally, before running `RFmerge` we need to define the output directory (`drty.out`) that will store each one of the merged files. Then, we can run the `RFmerge` function as follows:
...
If `RFmerge` run without problems, the following output files will be stored in your user-defined `drty.out` directory: 
...
Visualisation of one day of precipitation using RF-MEP, and overlying the boundaries of the study area (only its first attribute) and the gauging stations included in the independent evaluation dataset:

```{r, fig.width = 4, fig.height = 4, fig.align = "center", eval = TRUE}
plot(rfmep[[11]], main = "RF-MEP precipitation for 1983-01-11", 
...
To evaluate the performance of RF-MEP and the products used in its computation the modified Kling-Gupta Efficiency (*KGE'*) included in the `hydroGOF` pacakge will be used. The optimal value for the KGE is one. 
...
kge.table  <- data.frame(ID=eval.gis[["ID"]], CHIRPS=tmp, PERSIANN_CDR=tmp, RF_MEP=tmp)
...
```{r EvaluationPlotting, fig.width=7, fig.height=7, fig.align = "center", eval = TRUE}
...
boxplot(kge.table[,2:4], main = "KGE evaluation for Jan - Aug 1983",
        ...
        col=sres.cols)
legend("topleft", legend=c("CHIRPS", "PERSIANN-CDR", "RF-MEP"), col=sres.cols, 
...
2. Baez-Villanueva, O. M.; Zambrano-Bigiarini, M.; Beck, H.; McNamara, I.; Ribbe, L.; Nauditt, A.; Birkel, C.; Verbist, K.; Giraldo-Osorio, J.D.; Thinh, N.X. (2019). RF-MEP: a novel Random Forest method for merging gridded precipitation products and ground-based measurements, Remote Sensing of Environment (*accepted*).
