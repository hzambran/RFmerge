...
Finally, we will compare RF-MEP (and the two precipitation products used as covariates) with the rain gauge data from the evaluation set. To extract the RF-MEP precipitation values at the gauge locations, we will use the `extract` function from the `raster` package:
...
To evaluate the performance of RF-MEP and the products used in its computation, the modified Kling-Gupta Efficiency (KGE') will be used. The optimal value for the KGE' is one. The `KGE` function is included in the `hydroGOF` package. 


```{r EvaluationPlotting, fig.width = 7, fig.height = 5, fig.align = "center", eval= FALSE}
